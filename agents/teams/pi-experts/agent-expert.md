...
- `agents/*.md` — project-local (most common)
...
- Session files stored in `agents/sessions/`
...
Then read /tmp/pi-agent-ext-docs.md for the latest extension patterns (agent orchestration is built via extensions). Also search `agents/` for existing agent definitions and `extensions/` for orchestration patterns.
